...
class Google {
  ...
  async calculateTravelTime({
    ...
    try {
      ...
      const distance = {
        ...
        in_mile: Gdistance?.value
          ? Math.round((Gdistance?.value / 1000 / 1.60934) * 100) / 100
          : 0,
      };
  ...
  async getAddressDetails(lat, long) {
    ...
    try {
      ...
      const postalCodeObj = response?.data?.results[0]?.address_components.find(
        (component) => component.types.includes("postal_code")
      );
      const postalCode = postalCodeObj ? postalCodeObj.long_name : null;

      const countryObj = response?.data?.results[0]?.address_components.find(
